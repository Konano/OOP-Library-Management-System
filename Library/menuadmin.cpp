...
{
    static QString name=ui->lineEdit_6->text(),writer=ui->lineEdit_7->text(),ISBN=ui->lineEdit_9->text(),publisher=ui->lineEdit_8->text();
    static int total=ui->lineEdit_10->text().toInt();
    if (db.Check_Book_ISBN(ui->lineEdit_9->text()))
        {
...
void menuadmin::on_pushButton_28_clicked() //查找書籍
{
    QString name=ui->lineEdit_11->text(),writer=ui->lineEdit_12->text(),ISBN=ui->lineEdit_14->text(),publisher=ui->lineEdit_13->text();
...
//    for(int i=0;i < (int)BookList.size(); i++){
//        QString userID=QString::number(RecordList[i]->GetID());
//        QString totalnum=QString::number(BookList[i]->AvailableTotal());
//        QString lentnumb=QString::number(BookList[i]->LentTotal());
//        ui->tableWidget->setItem(i,0,new QTableWidgetItem(BookList[i]->GetName()));//要改成getName
//        ui->tableWidget->setItem(i,1,new QTableWidgetItem(BookList[i]->GetWriter()));//改getWriter
